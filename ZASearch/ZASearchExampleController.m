...
@interface ZASearchExampleController () <PYSearchViewControllerDelegate>

@end

@implementation ZASearchExampleController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PYSearchViewController *searchViewController = [PYSearchViewController searchViewControllerWithHotSearches:hotSeaches searchBarPlaceholder:@"搜索编程语言" didSearchBlock:^(PYSearchViewController *searchViewController, UISearchBar *searchBar, NSString *searchText) {
        // 开始搜索执行以下代码
        // 如：跳转到指定控制器
        [searchViewController.navigationController pushViewController:[[PYSearchViewController alloc] init] animated:YES];
    }];
    // 3. 设置风格
